...
# resume context so ticket-sized work can continue without a manual command.
#
...
# Project auto-resume wins. Tasks are fallback context for one-off ticket work.
...
No project WOS.md marker was found for this cwd, so Workflow OS is surfacing one-off task context.

To resume context, the model SHOULD call the memory-engine MCP:
  memory_search({ type: "task-state", project: "$task", limit: 1 })

Then summarize the task in 3-5 bullets before doing anything else this turn:
  status, Jira key, blocker/dependency, next action, and whether the user wants a Jira read.
"@
} catch { }
